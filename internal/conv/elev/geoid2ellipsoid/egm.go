...
func newDefaultEGM() (*egm, error) {
	return newEGM(defaultOrder, true)
}

func newEGM(nmax int, wgs84 bool) (*egm, error) {
	model := egm{
		nmax:  nmax,
		wgs84: wgs84,
	}
	...
	err := model.load()
	if err != nil {
		return nil, fmt.Errorf("error loading gravitational model data: %v", err)
	}
...
func (egm *egm) load() error {
	file, err := assets.GetAssets().Open("egm180.nor")
	if err != nil {
		return err
	}
	defer file.Close()
